...
function GanttRow({ staffId, name, tasks, axis, onSelectTask, onRemove, themeFilter, activeId, children, edgeHint, onEdgeHover }: { staffId: string; name: string; tasks: Task[]; axis: Date[]; onSelectTask?: (id: string) => void; onRemove?: (id: string) => void; themeFilter?: string | null; activeId?: string | null; children?: React.ReactNode; edgeHint?: { staffId: string; index: number } | null; onEdgeHover?: (info: { staffId: string; index: number } | null) => void }) {
  ...
  const getJunctionHoverState = (taskIndex: number) => {
    if (!edgeHint || edgeHint.staffId !== staffId) return false;
    // Check if this block is adjacent to the hovered junction
    return taskIndex === edgeHint.index || taskIndex === edgeHint.index - 1;
  };
    ...
    <div className={`gantt-row${isOver ? ' drag-over' : ''}`}>
      ...
      <div className="blocks">
        {tasks.map((t, taskIndex) => {
          ...
          const isJunctionHovered = getJunctionHoverState(start);
    ...
    onDragOver: (ev) => {
      ...
      if (overId.startsWith('gblock:')) {
        ...
        if (p) {
          ...
          // Check mouse position relative to the block to determine edge hover
          const overElement = document.querySelector(`[data-task-id="${tid}"]`) as HTMLElement;
          if (overElement && ev.activatorEvent instanceof MouseEvent) {
            const rect = overElement.getBoundingClientRect();
            const mouseX = ev.activatorEvent.clientX;
            const nearEdge = 15; // pixels from edge to trigger junction effect
            
            if (mouseX <= rect.left + nearEdge) {
              // Hovering near left edge - create space before this block
              setEdgeHint({ staffId: p.staffId, index: p.start });
            } else if (mouseX >= rect.right - nearEdge) {
              // Hovering near right edge - create space after this block
              setEdgeHint({ staffId: p.staffId, index: p.start + p.end });
            } else {
              // Hovering in middle - no junction effect
              setEdgeHint(null);
            }
          }
        } else {
          ...
          <div className="gantt-rows">
            {staffList.map((s, i) => (
              <GanttRow
                ...
                activeId={activeId}
                edgeHint={edgeHint}
                onEdgeHover={(info) => setEdgeHint(info)}
              >
