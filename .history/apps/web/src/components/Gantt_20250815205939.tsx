...
function GanttBlock({ task, staffId, start, end, leftPx, widthPx, themeFilter, onRemove, onEdgeHover }: { task: Task; staffId: string; start: number; end: number; leftPx: string; widthPx: string; themeFilter?: string | null; onRemove?: (id: string) => void; onEdgeHover?: (info: { staffId: string; index: number } | null) => void }) {
  ...
  React.useEffect(() => {
    ...
    } else if (!isDragging && !isDragEnding && isDragging !== undefined) {
      // Only set drag ending if we were actually dragging before
...
function GanttRow({ staffId, name, tasks, axis, onSelectTask, onRemove, themeFilter, activeId, children }: { staffId: string; name: string; tasks: Task[]; axis: Date[]; onSelectTask?: (id: string) => void; onRemove?: (id: string) => void; themeFilter?: string | null; activeId?: string | null; children?: React.ReactNode }) {
        ...
        {tasks.map((t) => {
          ...
          return (
            <div key={t.id}>
              ...
              <GanttBlock task={t} staffId={staffId} start={start} end={start + width} leftPx={leftPx} widthPx={widthPx} themeFilter={themeFilter} onRemove={onRemove} />
            </div>
          );
        })}
        {children}
  ...
  useDndMonitor({
    ...
    onDragOver: (ev) => {
      ...
      if (overId.startsWith('gblock:')) {
        ...
        if (p) setHint({ staffId: p.staffId, index: p.start }); else setHint(null);
          ...
          <div className="gantt-rows">
            {staffList.map((s, i) => (
              <GanttRow
                ...
                activeId={activeId}
              >
                {edgeHint && activeId && edgeHint.staffId === s.id ? (
                  <div className="drop-slot open" style={{ left: `calc(${edgeHint.index} * var(--cell-width))` }} />
