...
function GanttRow({ staffId, name, tasks, axis, onSelectTask, onRemove, themeFilter, activeId, children, edgeHint }: { staffId: string; name: string; tasks: Task[]; axis: Date[]; onSelectTask?: (id: string) => void; onRemove?: (id: string) => void; themeFilter?: string | null; activeId?: string | null; children?: React.ReactNode; edgeHint?: { staffId: string; index: number } | null }) {
    ...
    onDragOver: (ev) => {
      ...
      if (overId.startsWith('gblock:')) {
        ...
        if (p) {
          ...
          // Set edge hint for junction effect - check if we're near the end of the block
          const blockEnd = p.start + p.end;
          setEdgeHint({ staffId: p.staffId, index: blockEnd });
        } else {
          setHint(null);
          setEdgeHint(null);
        }
