...
app.post('/api/auth/request-login', async (req, reply) => {
  const body = z.object({ email: z.string().email() }).parse((req as any).body);
  ...
  const magicLink = `http://localhost:${serverPort}/login?token=${token}`;
...
app.delete('/api/staff/:id', { preHandler: requireAuth }, async (req: any, reply) => {
...
app.delete('/api/tasks/:id', { preHandler: requireAuth }, async (req: any, reply) => {
  const { id } = (req.params as any) as { id: string };
...
// Move / Reorder
app.post('/api/assignments/move', { preHandler: requireAuth }, async (req: any, reply) => {
  const body = z.object({
...
// Themes summary
app.get('/api/themes/summary', { preHandler: requireAuth }, async (req: any) => {
