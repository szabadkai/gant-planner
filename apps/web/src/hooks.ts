...
export function useBacklog() {
  return useQuery<Task[]>({ queryKey: ['tasks', 'backlog'], queryFn: api.listBacklog });
}
...
export function useAutoAssign() {
  ...
  const autoAssign = async () => {
    try {
      ...
      // Get fresh data
      const staff = await qc.fetchQuery({ queryKey: ['staff'], queryFn: api.listStaff });
      const backlogTasks = await qc.fetchQuery({ queryKey: ['tasks', 'backlog'], queryFn: api.listBacklog });
      ...
      alert(`Assigned ${backlogTasks.length} tasks evenly across ${staff.length} staff members`);
    } catch (error) {
