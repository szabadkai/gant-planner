...
export const api = {
  ...
  listProjects: (): Promise<Project[]> =>
    fetch('/api/projects', { headers: getAuthHeaders() }).then((res) =>
      j<Project[]>(res)
    ),
  ...
  listStaff: (): Promise<Staff[]> =>
    fetch('/api/staff', { headers: getAuthHeaders() }).then((res) =>
      j<Staff[]>(res)
    ),
  ...
  listBacklog: (): Promise<Task[]> =>
    fetch('/api/tasks?unassigned=true', { headers: getAuthHeaders() }).then(
      (res) => j<Task[]>(res)
    ),
  listTasksFor: (staffId: ID): Promise<Task[]> =>
    fetch(`/api/tasks?staff_id=${encodeURIComponent(staffId)}`, {
      headers: getAuthHeaders(),
    }).then((res) => j<Task[]>(res)),
  listAllTasks: (): Promise<Task[]> =>
    fetch('/api/tasks', { headers: getAuthHeaders() }).then((res) =>
      j<Task[]>(res)
    ),
  createTask: (input: {
    name: string;
  ...
  > =>
    fetch('/api/themes/summary', { headers: getAuthHeaders() }).then((res) =>
      j<{ theme: string; totalMandays: number; count: number }[]>(res)
    ),
  ...
  getSharedTasks: (
    token: string,
    params?: { staff_id?: string; unassigned?: boolean }
  ): Promise<Task[]> => {
  ...
  getSharedThemesSummary: (
    token: string
  ): Promise<{ theme: string; totalMandays: number; count: number }[]> =>
    fetch(`/api/share/${token}/themes/summary`).then(
      j<{ theme: string; totalMandays: number; count: number }[]>
    ),

  getSharedProject: (token: string): Promise<{ projectTitle: string | null }> =>
    fetch(`/api/share/${token}/project`).then(
      j<{ projectTitle: string | null }>
    ),
};
