...
function GanttBlock({
  ...
  onEdgeHover?: (info: { staffId: string; index: number } | null) => void;
  searchQuery?: string;
  isSelected?: boolean;
  ...
  const style: React.CSSProperties = {
    ...
    outline: isSelected ? '2px solid var(--color-brand-400)' : 'none',
    outlineOffset: '1px',
  ...
  return (
    <div
      ...
      ref={(el) => {
        setNodeRef(el);
        if (el) applyTheme(el as HTMLElement, task.theme);
      }}
...
export default function Gantt({
  ...
  const positions = useMemo(() => {
    const pos = new Map<
      string,
      { staffId: string; start: number; end: number }
    >();
    ...
    taskQueries
      .map((q, i) =>
        (q.data ?? []).map((t) => `${t.id}:${t.mandays}`).join(',')
      )
      .join('|'),
    activeId,
  ]);

  const [hint, setHint] = useState<null | { staffId: string; index: number }>(
    null
  );
  ...
  const axis = useMemo(
    () => buildAxis(startDate, horizon, skipWeekends),
    [startDate, horizon, skipWeekends]
  );
            ...
            {staffList.map((s, i) => (
              ...
              >
                {hint && hint.staffId === s.id ? (
                  <div
                    className='drop-hint'
                    style={{ left: `calc(${hint.index} * var(--cell-width))` }}
                  />
                ) : null}
              </GanttRow>
            ))}
