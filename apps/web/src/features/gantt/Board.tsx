...
import ThemesPanel from '../tasks/ThemesPanel';
...
export default function Board({
  ...
  const filteredBacklog = useMemo(() => {
    ...
    return (backlog ?? []).filter(
      (task) =>
        ...
        (task.theme &&
          task.theme.toLowerCase().includes(searchQuery.toLowerCase()))
    );
  }, [backlog, searchQuery]);
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'arrowup':
          ...
          if (allTasks.length > 0) {
            ...
            if (e.key === 'ArrowDown') {
              nextIndex =
                currentIndex < allTasks.length - 1 ? currentIndex + 1 : 0;
            } else {
              nextIndex =
                currentIndex > 0 ? currentIndex - 1 : allTasks.length - 1;
            }
  ...
  const containers = useMemo(() => {
    ...
    tasksQueries.map((q) => q.data?.length ?? 0).join(','),
  ]);

  const findContainerOf = (taskId: string): string | undefined => {
    for (const [cid, ids] of containers.entries())
      if (ids.includes(taskId)) return cid;
    return undefined;
  };
  ...
  const onDragEnd = (ev: DragEndEvent) => {
    ...
    if (typeof index === 'number') {
      if (to === from) {
        ...
        move({
          taskId: activeId,
          targetStaffId: to === 'backlog' ? null : to,
          beforeTaskId: before,
          afterTaskId: after,
        });
      } else {
    ...
    >
        ...
        >
          <div className='panel'>
            ...
            <div className='row' style={{ marginTop: 4 }}>
              <button
                onClick={async () => {
                  ...
                  a.href = url;
                  a.download = `export-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.csv`;
                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    Papa.parse(text, {
                      header: true,
                      skipEmptyLines: true,
                      complete: async (res: Papa.ParseResult<any>) => {
                        ...
                        const staffIndex = new Map<string, string>(
                          staffList.map((s) => [s.name.toLowerCase(), s.id])
                        );
                        for (const r of rows) {
                          const name = String(r.name || r.Name || '').trim();
                          const mandays = Math.max(
                            1,
                            Math.floor(Number(r.mandays || r.Mandays || 1))
                          );
            ...
            <Droppable id='list:backlog'>
              ...
              >
                {filteredBacklog.length === 0 && searchQuery.trim() ? (
                  <div
                    style={{
                      padding: 'var(--space-4)',
                      textAlign: 'center',
      ...
      {editing
        ? (() => {
            ...
            return task ? (
              <TaskDetailDrawer task={task} onClose={() => setEditing(null)} />
            ) : null;
          })()
        : null}
