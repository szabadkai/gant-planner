...
app.post('/api/auth/request-login', async (req, reply) => {
    ...
    return { message: 'Verification email sent! Check your email for the login link.' };
});
app.post('/api/auth/verify', async (req, reply) => {
    ...
    if (!verificationToken || verificationToken.usedAt) {
        return reply.status(400).send({ error: 'invalid_token', message: 'Invalid or expired token' });
    }
    if (verificationToken.expiresAt < new Date()) {
        return reply.status(400).send({ error: 'expired_token', message: 'Token has expired' });
    }
    // Mark token as used
    ...
    // Find or create user
    let user = await prisma.user.findUnique({ where: { email: verificationToken.email } });
    if (!user) {
...
app.get('/api/auth/me', async (req, reply) => {
    ...
    if (!userId) {
        return reply.status(401).send({ error: 'unauthorized', message: 'No user ID provided' });
    }
...
// Helpers
async function maxPosition(staffId) {
    const where = { staffId: staffId ?? null };
...
function nextPosition(base) {
    // leave gaps to reduce reindexing
    return base + 1024;
}
// Staff routes
app.get('/api/staff', { preHandler: requireAuth }, async (req) => {
...
app.post('/api/staff', { preHandler: requireAuth }, async (req, reply) => {
...
app.delete('/api/staff/:id', { preHandler: requireAuth }, async (req, reply) => {
    const { id } = req.params;
    // Verify staff belongs to user
    const staff = await prisma.staff.findUnique({ where: { id } });
    if (!staff || staff.userId !== req.userId) {
        return reply.status(404).send({ error: 'not_found', message: 'Staff not found' });
    }
...
app.post('/api/tasks', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!parse.success)
        return reply.status(400).send({ error: 'validation_error', issues: parse.error.issues });
...
// Move / Reorder
app.post('/api/assignments/move', { preHandler: requireAuth }, async (req, reply) => {
    const body = z.object({
    ...
    if (body.targetStaffId) {
        const staff = await prisma.staff.findUnique({ where: { id: body.targetStaffId } });
        if (!staff || staff.userId !== req.userId) {
            return reply.status(404).send({ error: 'not_found', message: 'Staff not found' });
        }
    }
...
// Themes summary
app.get('/api/themes/summary', { preHandler: requireAuth }, async (req) => {
...
app.post('/api/admin/clear', { preHandler: requireAuth }, async (req) => {
...
// --- Sharing ---
app.post('/api/share', { preHandler: requireAuth }, async (req, reply) => {
    const token = randomUUID();
...
async function ensureShare(token) {
